...
> 本文「所有读接口都是免鉴权的」这个前提,以及待办里「只存地址不存 token」,
> 经 [ADR-0016](0016-read-endpoint-auth.md) 修订(2026-07-30):服务端监听非
> loopback 地址时读接口需要 token,桌面端因此明文存一个令牌。
> 「不给服务端加 CORS」的结论不变 —— 理由在那里被加强,不是被推翻。

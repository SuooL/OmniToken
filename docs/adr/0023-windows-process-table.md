...
**但有一类会被丢掉的进程确实是真会话**,上线时实测到,记在这里而不是留给下一个
人再查一遍:

mypc 上同时开着两个 Claude Code 会话,agent 只报出一个。用同一份代码在两种上下文
里跑同一台机器的进程表:

| 运行上下文 | pid 637988(happy 守护起的) | pid 632160(OpenSSH 会话里起的) |
|---|---|---|
| SSH 登录(交互式) | 读到 | 读到 |
| 计划任务(与 agent 同样的 InteractiveToken) | 读到 | `OpenProcess: Access is denied` |

两个进程属主都是同一个用户,差别在 Windows 会话与完整性级别:OpenSSH 服务以
SYSTEM 派生用户进程,得到的是 session 0 的高完整性令牌,而计划任务里的 agent 是
session 1 的中完整性 —— 强制完整性控制不允许后者读前者的内存。这是 Windows 的
访问控制,不是可以绕过的实现细节(绕过要 SeDebugPrivilege,即管理员)。

后果的边界很窄:**在这台机器本地开的会话照常可见**,看不到的是「从别处 SSH 进来
再开」的那种。没有为它降级到映像名 —— 理由就是上一段,一个假阳性是永久的。

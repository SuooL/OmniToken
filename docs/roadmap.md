...
| 发布 job:desktop bundle | ✅ 完成 | `release.yml` 加 `desktop` job(macos-14,`needs: release`),同 tag 构建签名、生成 latest.json、挂到同一个 Release。2026-09-08 随 **v0.2.0** 首次真实发布跑通(8m37s),三个更新资产都挂上了 Release;本机 0.1.0 实例从真实端点自更新到 0.2.0,约 20 秒。验证中发现安装只替换 bundle、不动进程(pid 不变),已补 `app.restart()` |
